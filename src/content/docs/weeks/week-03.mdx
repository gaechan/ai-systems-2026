...
### Part 1: MIG — 컴퓨팅 격리 레이어

MIG가 물리적 격리를 제공하지만, **에이전트가 뭘 할 수 있는지는 MCP가 결정한다.** 이 절에서는 MIG의 핵심 개념을 짚고, Part 2부터 MCP를 심층 분석한다.
...
### Part 3: MCP 프리미티브 확장 — 서버 + 클라이언트 기능
...
MCP 최신 규격(2025-11-25)은 명시한다: **신뢰할 수 없는 서버의 도구 설명(description)도 untrusted로 취급**해야 한다. 도구 설명 자체에 프롬프트 인젝션이 내장될 수 있기 때문이다. 이것이 Part 5에서 다룰 McpInject 공격의 핵심 벡터다.
</Aside>
...
### Part 5: 보안과 거버넌스 — TBAC, OBO, Triple Gate
...
- [OpenAI MCP Guide](https://platform.openai.com/docs/guides/mcp) — 크로스벤더 채택 증거
